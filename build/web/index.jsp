...
<head>
    ...
    <style>
        :root { --primary: #0061ff; --dark: #1e293b; --accent: #38bdf8; --bg: #f8fafc; }
        body { font-family: 'Inter', 'Segoe UI', sans-serif; background-color: var(--bg); margin: 0; color: var(--dark); }
        .container { max-width: 1100px; margin: 0 auto; padding: 0 20px; }

        /* HERO SECTION */
        .hero { background: linear-gradient(135deg, #ffffff 0%, #e0f2fe 100%); padding: 100px 0; position: relative; overflow: hidden; }
        .hero-flex { display: flex; align-items: center; gap: 60px; }
        .hero-text { flex: 1.2; }
        .hero-title { font-size: 56px; font-weight: 800; line-height: 1.1; margin: 0 0 24px; color: var(--primary); }
        .hero-title span { color: var(--dark); }
        .hero-sub { font-size: 19px; color: #475569; margin-bottom: 40px; line-height: 1.6; }
        
        .btn-main { 
            background: var(--primary); color: white; padding: 18px 36px; border-radius: 50px; 
            font-weight: 700; text-decoration: none; display: inline-flex; align-items: center; gap: 12px;
            box-shadow: 0 10px 25px rgba(0, 97, 255, 0.3); transition: 0.4s; border: none; cursor: pointer;
        }
        .btn-main:hover { transform: translateY(-3px); box-shadow: 0 15px 30px rgba(0, 97, 255, 0.4); }
        ...
        .tag { background: #dbeafe; color: var(--primary); padding: 6px 14px; border-radius: 50px; font-size: 14px; font-weight: 700; }
        
        .doctor-card { 
            background: white; border-radius: 24px; padding: 30px; margin-bottom: 30px;
            display: flex; gap: 40px; align-items: center; transition: 0.4s;
            border: 1px solid #f1f5f9; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);
        }
        .doctor-card:hover { transform: scale(1.02); box-shadow: 0 20px 40px rgba(0,0,0,0.08); border-color: var(--primary); }
        
        .doc-avatar-wrap { position: relative; }
        .doc-img { width: 180px; height: 180px; border-radius: 24px; object-fit: cover; }
        .badge-exp { 
        ...
        .doc-info { flex: 1; }
        .doc-name { font-size: 26px; font-weight: 800; margin: 0 0 8px; color: var(--dark); }
        .doc-spec { color: var(--primary); font-weight: 700; font-size: 16px; margin-bottom: 12px; display: block; }
        .doc-desc { color: #64748b; line-height: 1.6; margin-bottom: 20px; font-size: 15px; }
        
        .doc-meta { display: flex; gap: 20px; align-items: center; }
        .meta-item { display: flex; align-items: center; gap: 6px; font-size: 14px; color: #475569; font-weight: 600; }
        
        .btn-book { 
            background: #f1f5f9; color: var(--dark); padding: 14px 28px; border-radius: 14px;
        ...
        .view-all-container { text-align: right; }
        .link-all { color: var(--primary); text-decoration: none; font-weight: 700; display: inline-flex; align-items: center; gap: 8px; cursor: pointer; }

        /* TOAST */
        .toast { position: fixed; bottom: 30px; right: 30px; background: #1e293b; color: white; padding: 16px 24px; border-radius: 16px; display: none; z-index: 1000; animation: slideIn 0.3s; box-shadow: 0 10px 20px rgba(0,0,0,0.2); }
            ...
            <div class="hero-text">
                ...
                <c:choose>
                    <c:when test="${empty sessionScope.account}">
                        <button class="btn-main" onclick="showToast(event)">
                            <i class="fas fa-calendar-check"></i> Đặt lịch hẹn ngay
                        </button>
                    </c:when>
                    <c:otherwise>
                        <a href="${pageContext.request.contextPath}/listofdoctorservlet" class="btn-main">
                            <i class="fas fa-calendar-check"></i> Đặt lịch hẹn ngay
                        </a>
                    </c:otherwise>
                </c:choose>
        ...
        <div class="container">
            <div class="section-header">
                ...
                <div class="view-all-container">
                    <c:choose>
                        <c:when test="${empty sessionScope.account}">
                            <a class="link-all" onclick="showToast(event)">Xem tất cả bác sĩ <i class="fas fa-chevron-right"></i></a>
                        </c:when>
                        <c:otherwise>
                            <a href="${pageContext.request.contextPath}/listofdoctorservlet" class="link-all">Xem tất cả bác sĩ <i class="fas fa-chevron-right"></i></a>
                        </c:otherwise>
                    </c:choose>
                </div>
            </div>
                ...
                <div class="doctor-card">
                    ...
                    <div class="doc-info">
                        ...
                        <div class="doc-meta">
                            ...
                            <div class="meta-item"><i class="fas fa-location-dot" style="color: #6366f1;"></i> ${doctor.clinicAddress}</div>
                        </div>
                    </div>
                    ...
                    <div>
                        <c:choose>
                            <c:when test="${empty sessionScope.account}">
                                <a href="#" class="btn-book" onclick="showToast(event)">Hẹn gặp bác sĩ</a>
                            </c:when>
                            <c:otherwise>
